...
public class BST 
{
	...
	while (leader.getValue() != remVal) {
	    if (leader == null) {
		return null; // don't change any values
	    }
	    else if (remVal > leader.getValue()) { // head toward the right subtree
		follower = leader;
		leader = leader.getRight();
	    }
	    else { // head toward the left subtree
		follower = leader;
